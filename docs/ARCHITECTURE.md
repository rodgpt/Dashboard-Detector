...
└──────────┬──────────┘   remote_config.json  │ + WAV archive        │
           │                                  └──────────┬───────────┘
           │ POST /api/devices/events                    │ reconcile (weekly,
           │ per-device credential, R-6.3                │ name-diff) and
           │ low-latency path — D-022                    │ one clip on demand
           │                                             ▼
           │                            ┌──────────────────────┐
           └───────────────────────────►│ BACKEND + FRONTEND   │
                                        │ FastAPI · Postgres   │
┌─────────────────────┐  HTTPS+session  │ 3 containers (D-019) │
│ BROWSER              │ ◄─────────────►│ derived event index  │
│ React SPA, served by │                │ answers every query  │
│ nginx container      │                └──────────────────────┘
└─────────────────────┘
```

**Two paths carry an event, and that is deliberate** (D-022). The push is fast and may fail. The blob is durable and is what the weekly reconcile checks the index against. Neither is a fallback for the other: the push is an optimisation, the blob is the record, and the index is derived from the blob and rebuildable from it (R-12.2).
...
**Storage** is the seam and the *durable* coupling — since D-022 it is no longer the only one, because the device also posts events to the API. The device writes to storage without the backend existing, and that property is deliberate and load-bearing: an unattended solar node must not depend on our uptime to record a detection. It is also why the blob write survives alongside the push. Storage is the archive too: clips at terabyte scale over years belong here, not in a database. Schemas: `docs/DATA-CONTRACT.md`, canonical in `Rpi-Detector`, mirrored here, enforced by validator and `make contract`.

**The second coupling is `POST /api/devices/events`** (R-6.3, D-022). It carries the same event the blob carries, for latency only. It is authenticated per device, it is allowed to fail, and nothing is lost when it does — the reconcile picks the event up from storage within the week. It is not in `DATA-CONTRACT.md` yet: a device→backend path that is not storage changes the coupling itself, and that starts in the device repository.
...
| Event push to the API fails | Blob still written. The weekly reconcile indexes it from storage; only freshness is lost, never the event (D-022) |
| Backend down while a device detects | Device records to storage as always, and spools the push. Nothing is lost; the dashboard is late, not wrong |
| Device stops reporting entirely | **Gap today.** `last_seen` goes stale and the badge shows it, but nobody is alerted. R-7.5 |
