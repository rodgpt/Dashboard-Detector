...
As of 2026-08-22 the **backend has converged; the device has not.**
...
| Version key | `version` | `config_version` | `config_version` |
| Signature covers | `{"version","config"}` only | whole document minus `signature` | whole document minus `signature` |
| Key name | `OCEANKIND_CONFIG_HMAC_KEY` | `OCEANKIND_CONFIG_HMAC_KEY` | `OCEANKIND_CONFIG_HMAC_KEY` |
| No key configured | applies unsigned with a warning | refuses to publish, 503 | refuses to apply |
| v1 flat key names | still accepted | not accepted | not accepted |

**Four things remain on the device side**, and until they land every published document fails its signature check there, silently — nothing logs an incompatibility, configuration simply stops applying:

1. Read `config_version`, not `version`.
2. Canonicalise the **whole document minus `signature`**, not `{"version","config"}`. Two sides hashing different objects is the whole failure.
3. Refuse to apply when no key is configured. Applying unsigned with a warning is the F-10 hole reopening.
4. Stop accepting v1 flat key names.

The backend side is verified: the published blob verifies against an independent HMAC recompute, `config_version` is an opaque string that changes on every tune, there is no `expires_utc`, and out-of-range values are clamped and reported before signing.
