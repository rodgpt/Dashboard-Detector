# Data contract v2 — target design

**Status: design. Not built, not deployed.** Until v2 ships, the device emits the **v1 layout plus the Phase 1 additions documented in the next section**, which are authoritative for what is on the wire today.
...
## As-built: v1 + Phase 1 additions (2026-08-12) — ON THE WIRE

Phase 1 (fail-loud) changed what the device emits, additively, still on the v1 paths (`status.json`, `manifest.json`, `power_history.json`, flat or site-prefixed). The dashboard must tolerate all of it; the new fields are the ones its Phase 1 tasks consume.
...
Live units are writing v1 to the old paths right now. The client owns deployment, so this is a handover procedure rather than something we execute.

Write v2 alongside v1 for one release, so nothing breaks while the dashboard still reads v1. Dashboard moves to v2 with a v1 fallback. Backfill history into `sites/{id}/events/` from the existing manifests, one blob per entry, with `event_type: "unknown"` and `detector: "unknown"` for everything before the detector swap, since their meaning is genuinely unknown. Then drop the v1 writes.

The `?play=` deep links already sitting in people's WhatsApp history must keep resolving throughout. Simplest is a redirect from the old bare blob name to the new clip path.
