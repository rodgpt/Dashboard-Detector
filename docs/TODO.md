...
- [x] **[Sec] Bootstrap could create an administrator that can never log in** DONE (2026-08-13) — `OCEANKIND_BOOTSTRAP_ADMIN_EMAIL` was written to the database unvalidated, but login validates with `EmailStr`, so a reserved-domain address (e.g. `admin@x.local`) bootstrapped a dead admin silently. `init_db` now refuses to start on an invalid bootstrap email. *The 12-character bootstrap password minimum that shipped with this was removed 2026-09-04 — it turned a first deployment into a restart loop over a value the operator chose deliberately. The email check stays: a malformed address creates an admin who cannot sign in, and there is no password-change route to recover with.*
