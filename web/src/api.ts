...
export interface Site   { id: string; name: string; lat: number; lon: number;
                          device: string; active: boolean; }

export interface Clip   { path: string; sample_rate: number; channels: number;
...
export interface DetectionEvent {
  ...
  event_type: EventType; detector: string; score: number; suppressed: boolean;
...
export interface Page<T> { items: T[]; total: number; limit: number;
                           offset: number; has_more: boolean; scanned_blobs: number; }

export interface Health { detector_ok: boolean; audio_ok: boolean;
                          duty_cycle_pct: number | null; clips_dropped: number;
                          upload_backlog: number; degraded_reason: string | null; }

export interface Me { email: string; role: "operator" | "admin"; sites: string[]; }

export interface AdminUser { id: number; email: string; role: "operator" | "admin";
                             active: boolean; sites: string[]; }

export interface AdminDevice { id: number; device_id: string; site_id: string;
...
export const auth = {
  login:  (email: string, password: string) =>
            req<{ ok: true }>("/auth/login", { method: "POST",
...
export const data = {
  ...
  status:   (s: string) => req<Record<string, any>>(`/sites/${s}/status`),
  power:    (s: string) => req<Record<string, any>>(`/sites/${s}/power`),
  acoustic: (s: string) => req<Record<string, any>>(`/sites/${s}/acoustic`),
  ocean:    (s: string) => req<Record<string, any>>(`/sites/${s}/ocean`),
...
export const admin = {
  ...
  createUser: (b: { email: string; password: string; role: "operator" | "admin"; sites: string[] }) =>
                req<AdminUser>("/admin/users", { method: "POST",
                  headers: { "content-type": "application/json" }, body: JSON.stringify(b) }),
  setSites:   (id: number, sites: string[]) =>
                req<AdminUser>(`/admin/users/${id}/sites`, { method: "PUT",
                  headers: { "content-type": "application/json" }, body: JSON.stringify({ sites }) }),
